...
import '../widgets/callBy_drowdown.dart';
import '../dialogueBox/candidate_details_dialogue_box.dart';
import '../widgets/recruiter_dropdown.dart';

class CallCandidateTable extends StatefulWidget {
  final String jobId;
  ...
  @override
  State<CallCandidateTable> createState() => _CallCandidateTableState();
}

class _CallCandidateTableState extends State<CallCandidateTable> {
  ...
  //       .where('callStatus', isEqualTo: null) // Filter by resumeStatus
  //       .snapshots()
  //       .map((snapshot) {
  ...
  Stream<List<Map<String, dynamic>>> fetchApplications(String jobId) {
    return FirebaseFirestore.instance
      ...
      return snapshot.docs.where((doc) {
        final callStatus = doc['callStatus'];
        return callStatus == null || callStatus == 'Not-Received'; // Filter out if callStatus is "Selected"
      }).map((doc) {
        final data = doc.data() as Map<String, dynamic>;
        data['applicationId'] = doc.id; // Add the document ID
        return data;
                          ...
                          child: Row(
                            children: [
                              ...
                              SizedBox(
                                  width: 150,
                                  child:
                                  CallByRecruiterDropdown(
                                    initialValue: rowData['callBy'], // Pass the current recruiter value
                                 ...
                                 // ),
                               //   child: Text(rowData['recruiter'] ?? "No", textAlign: TextAlign.center,
